...
- **Blueprint: from a requirement to a harness design.** A guide and a builder that turn a client requirement into a high-level design (the job, a checkable “done”, systems and sources of truth, autonomy per action, risks, loop shape) and a low-level design (tool table with tiers, idempotency keys, timeouts and undo; a decision for each of the 20 concepts), then trace every sentence to an edge case and a threshold. A rule-based highlighter marks the phrases that imply harness work (“under ₹5 lakh”, “approved by”, “never twice”, “by email”). Exports a client proposal, a design doc, an eval checklist (CSV) and a harness-hub domain skeleton. Every harness page also gets a Blueprint tab with its own design, reverse-engineered, where each row opens the run at the moment it mattered. Everything stays in your browser.
